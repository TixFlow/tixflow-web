    ...
    <div key={index} className="my-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-4">
        {section.events.map((event, eventIndex) => (
          <div
            key={eventIndex}
            className="event-card relative hover:shadow-lg transition-shadow duration-300"
          >
            <img
              src={event.image}
            ...
            <div className="absolute bottom-0 bg-opacity-60 bg-black text-white p-2 rounded-b-lg w-full text-center">
              ...
              {event.button && (
                <button
                  className="bg-white text-black px-3 py-1 rounded-full mt-2 text-sm font-semibold"
                  aria-label={`Button for ${event.title}`}
                >
                  {event.button}
                </button>
              )}
